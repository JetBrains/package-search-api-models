...
import org.jetbrains.packagesearch.api.v3.revision1.search.PackagesType

@Serializable
public data class GetPackageInfoRequest(
    public val requests: List<CacheRequest> = emptyList(),
) {
    @Serializable
...
public fun GetPackageInfoRequest(ids: List<String>): GetPackageInfoRequest {
    return GetPackageInfoRequest(ids.map { GetPackageInfoRequest.CacheRequest(it) })
}
